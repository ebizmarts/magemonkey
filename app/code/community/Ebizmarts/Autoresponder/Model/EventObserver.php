...
class Ebizmarts_Autoresponder_Model_EventObserver
{
    ...
    public function orderSaved(Varien_Event_Observer $observer){
        ...
        if (array_key_exists('status', $new_data) && $new_data['status'] && $original_data['status'] !== $new_data['status'] && $new_data['status'] == Mage::getStoreConfig(Ebizmarts_Autoresponder_Model_Config::NEWORDER_ORDER_STATUS, $storeId)) {
